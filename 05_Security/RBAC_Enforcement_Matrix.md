...
### 7.4 Dashboard decision (final, updated BE-068)

`GET /reports/dashboard` still requires only `report.view` at the endpoint-permission level (`apps/dashboard/views.py::DashboardView.permission_code`) — a PM/Designer-type role that never held `report.financial_access` must still reach the operational sections, so the endpoint itself isn't re-gated. `report.financial_access` remains required, unchanged, for `GET /reports/finance` and `GET /reports/expenses`.

**BE-068 resolved:** the response payload is now split at the source (`DashboardService.compute`'s `include_financial` flag, resolved per-request from the caller's real `report.financial_access` permission via `PermissionService.has_permission` — never a role-name check, and platform admin keeps its existing standing bypass). A caller without `report.financial_access` never receives, in any form (not null, not zero): `kpis.totalBilledRevenue`/`.totalReceived`/`.pendingAmount`/`.totalExpenses`/`.netProfitLoss`, or the `pendingPayments`/`overdueInvoices`/`recentExpenses`/`projectProfitability` sections — those keys are absent from the JSON entirely (DRF `SkipField`, via `required=False` fields with no default).

`recentActivities` — despite reading as operational ("who did what, when") — is gated the same way. Confirmed by direct testing during this fix, not assumed: `AuditLogSerializer`'s `beforeState`/`afterState` are raw snapshots of whatever entity an audit entry touched, and an invoice/payment audit entry's snapshot contains real `total`/`amount` figures. Redacting only the financial fields from individual entries would need new per-entity-type filtering in the audit layer; gating the whole section was the smallest change that actually closes the leak — recorded here since it's a real, deliberate narrowing of what `report.view`-only roles see, beyond what this section previously documented as "operational."

A `canViewFinancials: boolean` field accompanies the response as a frontend convenience — it never substitutes for the omission; the financial keys are already genuinely absent regardless of its value. `recentQuotations[].total` (a real currency figure) remains visible to `report.view` alone — a quotation's status/value is treated as project-management-relevant information distinct from company financial performance data, per this task's own explicit classification, and was not found to appear anywhere else in a way that would contradict that.
...
### 7.7 Migration safety review — **resolved by BE-069** (was: raised per explicit Backend Lead audit request, not yet resolved)

**Historical finding (superseded):** `0006_backfill_legacy_member_role.py`, `0007_reconcile_admin_accountant_permissions.py`, and `0008_grant_report_view_to_operational_roles.py` all identify their target role(s) **by exact `name` match**. `Role` had no field distinguishing a platform-seeded default role from a company's own hand-created custom role. Those three migrations are historical and already applied — left unmodified, per BE-069's own scope (rewriting an already-applied migration is unsafe; the fix is forward-looking).

**Resolution (BE-069):** `Role.system_key` — a nullable, `editable=False`, never-API-settable `CharField` unique per company (`unique_system_key_per_company`, `condition=Q(system_key__isnull=False)`) — is now the sole machine identity for a default/system role. `RoleService.seed_default_roles_for_company` sets it at seed time from `apps.users.permission_catalog.DEFAULT_ROLE_SYSTEM_KEYS`; no other code path may set it. A customer-created role — including one a customer names `"Owner"`, `"Admin"`, or anything else exactly matching a default role's display name — always gets `system_key = NULL` and therefore never receives system-role semantics. Renaming a role (still fully permitted) never touches `system_key`, so identity survives a rename in either direction.

**Historical data:** a new migration (`0010_backfill_role_system_key.py`) backfills `system_key` onto already-seeded roles from before this field existed, using three independent, deterministic signals combined (never display name alone): (1) name matches a documented default role, (2) the role was created within seconds of its own company (proven, both architecturally and empirically against this project's live database, to be how `seed_default_roles_for_company` actually behaves — a hand-created role is practically never this fast), (3) no `AuditLog` CREATE row exists for it (the real `POST /roles` path always writes one; seeding never does). Any role failing even one signal is left `system_key = NULL` — the deliberately safe default, since under-classifying a real system role only loses a protection an operator can restore by hand, while over-classifying a genuine custom role would incorrectly grant it undeletable/last-owner protections it has no right to. See the migration's own docstring for the full reasoning, and `BACKEND_TASKS.md` BE-069 for verification against this project's actual data (all 6 pre-existing default-named roles classified correctly, zero ambiguous cases).

**Protected system role:** only the Owner role (`system_key == "owner"`) is protected — undeletable (`RoleService.soft_delete_role` raises `ConflictError`/`SYSTEM_ROLE_PROTECTED`), and a company must always retain at least one `ACTIVE` membership holding it (`CompanyMembershipService._reject_if_would_remove_last_active_owner`, `ConflictError`/`LAST_OWNER_REQUIRED`, enforced on role-change/suspend/remove, concurrency-safe via `SELECT ... FOR UPDATE` on the company's active-Owner membership rows). No other default role (Admin, Project Manager, etc.) is protected — no documented product requirement supports it, and none was invented.

**§4/§7.6 forward-looking note:** any future default-role reconciliation migration must match target roles by `system_key`, never `name` — the pattern `0006`–`0008` used is now the documented anti-pattern this section exists to warn against, not a precedent to repeat.
