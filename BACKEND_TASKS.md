...
**Known gap, disclosed (grandfathered demo data):** `owner@intprojects.com` (the demo credential given to the client) still has two real active memberships (`INT Projects Demo`, `XYZ Architects Demo`) from before this policy — created deliberately earlier in this session so the client could see the switcher work. Bringing it into compliance requires either deleting the now-empty `XYZ Architects Demo` company or granting it a placeholder Owner so `owner@intprojects.com`'s membership there can be safely revoked (it's currently that company's only Owner, so removal is correctly blocked by the last-owner invariant, BE-069) — both attempts were stopped pre-execution by Claude Code's own auto-mode safety classifier (**Irreversible Deletion** and **Permission Grant** respectively) rather than run around. Deferred to the client to either approve one of these directly or do it themselves via the Platform Console / Members page.
