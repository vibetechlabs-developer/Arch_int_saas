...
| BE-069 | Stable system role identity (`Role.system_key`) + protected Owner role + last-active-owner invariant — see writeup below. | **Review** |
...
**Priority:** P0/security — closes the last identified RBAC hardening gap; reconciles the debt this ID has tracked since BE-054's migration safety review, rather than opening a duplicate.

**Owner:** Backend Team

**Audit before coding:** confirmed via direct grep that zero existing code anywhere identified an "owner" by anything other than the seeded role's display name being `"Owner"` — no `is_owner`/`last_owner` logic existed at all (BACKEND_TASKS.md's own BE-071 writeup explicitly deferred this: *"Last-owner protection was explicitly NOT attempted... any 'who is the current owner' check would have to match the free-text display name"*). Confirmed `unique_active_role_per_company` (on `company`+`name`, active rows only) makes a *simultaneous* same-named duplicate role impossible — the real, reproducible ambiguity is rename-then-recreate: rename the real Owner role away, then create a brand-new custom role literally named `"Owner"`. Confirmed `seed_default_roles_for_company`'s own docstring: default roles are **not** backfilled onto companies that existed before that feature shipped, so a blind name-match backfill would misclassify custom roles in those companies.

**Stable identity:** `Role.system_key` — nullable `CharField`, `editable=False`, `unique_system_key_per_company` (`UniqueConstraint` on `company`+`system_key`, `condition=Q(system_key__isnull=False)`). Derived once, deterministically, from each documented default role name (`apps/users/permission_catalog.py::DEFAULT_ROLE_SYSTEM_KEYS`, e.g. `"Owner"` → `"owner"`) and set **only** by `RoleService.seed_default_roles_for_company` at company-creation time. Never accepted from any API input — `RoleCreateSerializer`/`RoleUpdateSerializer` simply have no field for it, so a request body cannot inject it regardless of what it sends (verified directly: `test_create_role_ignores_system_key_in_request_body`/`test_update_role_cannot_inject_system_key_onto_a_custom_role`). Renaming a role (still fully permitted, unchanged) never touches `system_key` — identity survives rename in both directions, confirmed live.

**Historical backfill — migration safety (Phase 4/23, explicit reporting required):** `0010_backfill_role_system_key.py` backfills already-seeded roles using **three independent, deterministic signals combined, never display name alone**: (1) name matches a documented default role, (2) `role.created_at` within 10s of its own `company.created_at` (`seed_default_roles_for_company` runs inside the same transaction as company creation — confirmed empirically against this project's live database: observed deltas of 0.004s–0.033s, two orders of magnitude inside the tolerance), (3) no `AuditLog` CREATE row exists for the role (the real `POST /roles` path always writes one; seeding never does). Any role failing even one signal is left `system_key = NULL` — the deliberately safe default (under-classifying loses a protection that can be restored by hand; over-classifying would incorrectly grant undeletable/last-owner protections to a genuine customer role). **Verified against this project's actual database**: all 6 pre-existing default-named roles satisfied all three signals and were correctly backfilled; roles named `"Company Owner"`, `"Legacy Member"`, and several test-fixture custom roles sharing no default name were correctly left untouched. Idempotent and reversible (best-effort, matching `0006`/`0007`'s own precedent). **Migration safety: PASS** — not blocked, since a deterministic (non-name-only) signal was available; see the migration's own docstring for the full reasoning.

**Protected role:** only the Owner role (`system_key == "owner"`) is protected — `RoleService.soft_delete_role` now raises `ConflictError`/`SYSTEM_ROLE_PROTECTED` (409) for it, and only it. No other default role (Admin, Project Manager, etc.) is protected — no documented product requirement supports extending this, and none was invented.

**Last-active-Owner invariant:** a company must always retain at least one `ACTIVE` membership holding the Owner role. Enforced in `CompanyMembershipService` at every mutation path that could violate it — `assign_role` (change/clear), `_set_status` (suspend, i.e. the existing revoke path), `remove_member` — via a new shared guard, `_reject_if_would_remove_last_active_owner`, raising `ConflictError`/`LAST_OWNER_REQUIRED` (409). Identified via `membership.role.system_key`, never `membership.role.name`. Self-removal/self-suspension of the last Owner is still separately blocked by the pre-existing (BE-071) unconditional self-action guard, which fires first. A revoked/inactive Owner membership never counts toward the invariant. Assigning the very first Owner to a freshly created company (0 → 1 active owners) is correctly never blocked — the guard only fires when the CURRENT membership is already an active Owner.

**Concurrency safety (Phase 8):** `CompanyMembershipRepository.lock_active_role_membership_ids` locks (`SELECT ... FOR UPDATE`) every currently-active Owner membership row for the company before counting, inside the caller's existing `transaction.atomic()` block — PostgreSQL rejects `FOR UPDATE` combined with an aggregate, so this returns a materialized id list and callers use `len()`, never `.count()`. Two concurrent requests demoting/removing two *different* Owners correctly serialize: whichever transaction commits first succeeds, the second re-evaluates the now-current (reduced) count and is correctly rejected. Verified with a dedicated `TransactionTestCase` forcing the race window open deterministically (real threads, real separate connections, an artificial delay held *while the lock is held*) — exactly one of two concurrent demotions succeeds, the company always retains ≥1 active Owner.

**Custom role named "Owner":** thoroughly proven never protected — `test_custom_role_named_owner_is_not_system_owner`, `test_custom_role_named_owner_never_triggers_last_owner_protection`, `test_custom_role_named_owner_can_be_deleted_normally`, plus a live HTTP reproduction (rename real Owner away, create a new "Owner"-named role, confirm it has no `systemKey` and none of the real Owner's protections).

**Privilege escalation / Add User / multi-company isolation:** all regression-tested directly against the new code paths, not merely by re-running old suites — `test_privilege_escalation_guard_still_blocks_assigning_owner_role` (the pre-existing BE-054 escalation guard still runs and still blocks, unaffected by the new last-owner check), `test_add_user_into_owner_role_still_works`, `test_owner_status_is_isolated_per_company` (a user who is the sole Owner of Company 1 and merely an Admin of Company 2 is protected in the first, freely removable in the second).

**API contract:** `RoleSerializer` gained `systemKey` (nullable string) and `isSystem` (convenience boolean, `systemKey !== null`) — both read-only. `CompanyMembershipSerializer` gained `roleSystemKey` (nullable string, sourced from `role.system_key`, no extra query — `role` was already `select_related`). Neither `RoleCreateSerializer` nor `RoleUpdateSerializer` gained any field for it.

**Tests:** `apps/users/tests/test_role_system_key.py` (17), `test_last_owner_protection.py` (16), `test_last_owner_concurrency.py` (1), `test_last_owner_api.py` (9) — 43 new. Focused regression (`apps.users apps.authentication apps.company apps.audit`): see final report. Full backend suite: see final report.

**Validation:** `manage.py check` clean. New migrations `0009`/`0010` applied and verified live against real seeded/renamed/recreated roles (see report's Live HTTP verification section).
...
#### BE-061 — Leads/CRM Module — 2026-09-15

**Status:** Review (awaiting Backend Lead approval — not self-approved)

**Priority:** P3 (per `CLAUDE.md`'s own MVP phasing) — pulled forward; see Governance note below.

**Owner:** Backend Team

**Governance note (explicit disclosure, not a silent skip):** `CLAUDE.md` ("Do not build out Phase 3–5 features... unless the user explicitly asks to jump ahead"), `03_Database/Migration_Plan.md`'s header, and `09_Project/Roadmap.md` ("Do not pull Phase 3+ features forward without client confirmation") all gate this module behind Phases 1–2 being solid and/or explicit client scheduling. This task proceeded on the strength of the product owner's own explicit in-session instruction to continue building the remaining SaaS modules beyond Phases 1–2 — a real-time authorization substituting for the normally-required Roadmap scheduling step, not a decision made unilaterally by this task. Flagged here transparently, matching this project's standing practice around BE-069's own disclosed scope decisions.

**Audit before coding:** `01_Business/FRS.md §8` documents the Lead flow itself (`Lead → Qualification → Follow-up → Site Visit → Won → Client → Project`, lost leads need a loss reason + optional re-follow-up date) but no field list or exact status vocabulary — unlike Client's explicit "Name, Company, Email, Mobile, GSTIN". `04_API/CRM_API.md` sketches `GET /leads` (filter: status, assigned to), `POST .../mark-lost` (loss reason required, optional follow-up date), `POST .../convert` (idempotent, auditable). Audited `apps/clients` as the closest architectural precedent (a Lead converts into a Client verbatim) and mirrored its exact View→Serializer→Service→Repository/Selector→Model layering; audited `apps/projects`' status-transition-graph pattern (`get_allowed_next_statuses`, dedicated `PATCH .../status` action) as the template for Lead's own status machine.

**Field set (Backend Lead decision, since none is documented):** `name`/`company_name`/`email`/`mobile` mirror Client's own identity fields exactly (avoids a lossy/invented mapping at conversion time). `source` is free text (no invented closed vocabulary). `assigned_to` is directly implied by CRM_API.md's own `GET /leads` filter list, not invented. `loss_reason`/`follow_up_reminder_at` are explicitly documented requirements. No `gstin`/`addresses` — documented Client-only fields, not part of a lead's pre-qualification data.

**Status vocabulary (Backend Lead decision, literal transcription of the documented flow):** `LeadStatus`: `new, qualified, follow_up, site_visit_scheduled, won, lost`. `MAIN_CHAIN_STATUSES` stops at `site_visit_scheduled` — `won` is deliberately excluded from `get_allowed_next_statuses`' return set entirely and is reachable **only** via `POST /leads/{id}/convert`, never the plain `PATCH .../status` endpoint, since converting has real side effects (creating a real Client, optionally a Project) that a bare status flip must never be able to skip. `lost` is a side-transition reachable from any non-terminal status (mirrors Project's `ON_HOLD`/`CANCELLED` side-state pattern), not part of the main chain.

**Endpoints:** `GET/POST /leads`, `GET/PATCH/PUT/DELETE /leads/{id}` (CRUD, mirrors `ClientViewSet` exactly — PUT behaves like PATCH, matching the existing RoleViewSet/ClientViewSet convention), `PATCH /leads/{id}/status` (plain transition), `POST /leads/{id}/mark-lost` (requires `lossReason`, optional `followUpReminderAt`), `POST /leads/{id}/convert` (optional `createProject`/`projectName`). URL wiring uses the same manual `re_path` + `.as_view({...})` dict-mapping pattern as `apps/projects/urls.py` (confirmed by reading it first) — this codebase does not use DRF's `DefaultRouter`.

**Convert (idempotency + transactional integrity):** `LeadService.convert_lead` — if `status == won` and `converted_client_id` is already set, returns the lead unchanged (no duplicate Client/Project ever created on a repeat call, exactly matching CRM_API.md's own documented idempotency requirement). Rejects a `lost` lead with 409 (`"A lost lead cannot be converted."`) — no "revive a lost lead" flow is documented, so none is invented. Otherwise creates a real `Client` via `ClientService.create_client` (using the lead's own name/company_name/email/mobile, with a note recording the conversion and original source) and, if `createProject=true`, a real `Project` via `ProjectService.create_project` (lazy-imported to avoid a circular import), carrying the lead's `assigned_to_id` forward — then sets `status=won` + `converted_client`(+`converted_project`) in the **same transaction** as the Client/Project creation, so a lead can never end up "won" without a real `converted_client` behind it. One `AuditAction.UPDATE` log entry per real conversion (none on the idempotent no-op path, verified by test).

**Permissions:** 5 new codes added to `apps/users/permission_catalog.py`'s `PERMISSION_CATALOG` — `lead.view`, `lead.create`, `lead.edit` (also covers status-transition/mark-lost, matching Project's `project.edit` covering its own status endpoint), `lead.delete`, `lead.convert` (its own distinct code, not folded into `lead.edit`, so a role can convert without being able to delete). `DEFAULT_ROLE_PERMISSIONS` extended: Admin gets all 5; "Sales / CRM User" gets view/create/edit/convert but **not** delete. New data migration `apps/users/migrations/0011_seed_lead_permissions.py` seeds the 5 `Permission` rows and reconciles grants onto any **already-seeded** Admin/Sales-CRM-User role for pre-existing companies (mirrors `0007_reconcile_admin_accountant_permissions`'s exact pattern) — `RoleService.seed_default_roles_for_company` only auto-grants new codes to companies created *after* this migration, so existing companies needed this explicit backfill. Verified live: `Permission.objects.filter(module="lead")` returns all 5 codes; an existing seeded company's Admin role gained all 5, its Sales/CRM role gained 4 (no delete).

**Audit logging:** `apps/audit/validators.py`'s `ENTITY_FIELD_ALLOWLISTS` gained a `"lead"` entry (`name`, `company_name`, `email`, `mobile`, `source`, `status`, `assigned_to_id`, `loss_reason`, `follow_up_reminder_at`, `converted_client_id`, `converted_project_id`) — discovered as a real gap during this task's own test run (an entity type with no registered allowlist silently persists an **empty** state dict rather than raising, by design — a caller-side safety net, not a hard failure — so this was caught by an assertion on actual audit content, not by a crash). `notes` deliberately excluded, same privacy/payload-size reasoning as Client's own exclusion (BE-023).

**Tenant isolation / RBAC:** `LeadPermission(TenantScopedPermission)` mirrors `ClientPermission` exactly — cross-tenant access returns 404 (not 403, matching `Error_Handling.md §5`), verified indistinguishable from a genuinely nonexistent lead. Platform admin bypass verified on every endpoint including the 3 new action routes.

**Tests:** `apps/leads/tests/{test_models,test_services,test_permissions,test_views,test_workflow}.py` — 124 new tests (model lifecycle/soft-delete/cascade, the pure transition-graph function, full service-layer coverage including convert idempotency and cross-tenant scoping, unit-level permission checks, full CRUD API-level tests including cross-tenant IDOR/403/404 parity, status/mark-lost/convert endpoint tests, and granular RBAC tests proving `lead.convert` is enforced independently of `lead.delete`). All 124 passed (`manage.py test apps.leads --keepdb`, 387s). Focused regression (`apps.users apps.clients apps.projects apps.audit apps.company`, the apps whose shared files this task touched): **571/571 passed**, 2036s (34m) — zero regressions from the new `lead.*` permission codes or the `apps/audit/validators.py` allowlist addition.

**Validation:** `manage.py check` clean. `makemigrations --check --dry-run`: no changes detected. New migrations `apps/leads/migrations/0001_initial.py` and `apps/users/migrations/0011_seed_lead_permissions.py` applied and verified live against a real running server — full CRUD, the entire status-transition graph (valid forward move, skip-ahead 409, direct-to-`won` 409), mark-lost (missing-reason 400, success, already-lost 409), convert (with `createProject`, verified idempotent re-call returns the identical `convertedClientId`/`convertedProjectId`, lost-lead 409), cross-tenant/nonexistent 404 parity, and unauthenticated 401 — all seeded test data (company, user, membership, lead, client, project) hard-deleted afterward and confirmed gone; server confirmed stopped.

**Files changed (backend):** `apps/leads/` (new app: `models`, `admin`, `repositories`, `validators`, `selectors`, `services`, `permissions`, `serializers`, `views`, `urls`, `migrations/0001_initial.py`, `tests/*`), `apps/users/permission_catalog.py` (5 new codes + role grants), `apps/users/migrations/0011_seed_lead_permissions.py` (new), `apps/audit/validators.py` (`"lead"` allowlist entry), `config/settings.py` (`INSTALLED_APPS`, `ENUM_NAME_OVERRIDES`), `config/urls.py` (wired `apps.leads.urls`). Docs: `03_Database/Migration_Plan.md` (028 row + Deviations entry), `04_API/CRM_API.md` (Lead section marked implemented, as-built paths/permissions documented).

#### BE-062 — Site Visit Module — 2026-09-15

**Status:** Review (awaiting Backend Lead approval — not self-approved)

**Priority:** P3 (per `CLAUDE.md`'s own MVP phasing) — pulled forward; same Governance note as BE-061 above.

**Owner:** Backend Team

**Governance note:** identical basis to BE-061's own — built on the product owner's explicit in-session instruction to continue past Phases 1–2, not a unilateral decision. See BE-061's writeup for the full reasoning; not repeated here.

**Audit before coding:** `01_Business/FRS.md §9` documents a real field list for this entity ("client, project/lead, visit date, assigned person, address, measurements, requirements, photos, videos, notes, budget, site conditions, follow-up actions") and that it "produces a Site Visit Report that can create/update a project" — unlike Lead, so the field set here is a direct transcription, not a Backend Lead invention. `03_Database/ER_Diagram.md` §2 confirms a site visit is schedulable against either `LEAD` or `PROJECT` (both relations shown, both optional). `04_API/CRM_API.md`'s own sketch (`GET/POST /site-visits`, `PATCH .../{id}`, `POST .../{id}/report`) was used directly for the endpoint shape. Audited `apps.leads` as the closest precedent (same Backend-Lead-decision-documentation discipline, same tenant/RBAC/audit layering).

**Scope decisions requiring disclosure (Backend Lead decisions, since FRS names the concepts but not their structure):**
- **No `status` field.** FRS/CRM_API.md document no status vocabulary for Site Visit (unlike Lead, whose FRS entry names its flow stages explicitly). `report_submitted_at` (nullable) alone carries the scheduled/completed distinction — inventing a parallel enum to say the same thing would be exactly the kind of undocumented vocabulary this codebase's own Lead module already avoided.
- **`photo_urls`/`video_urls` are plain JSONField URL lists, not real storage-backed uploads.** `Document.project` (the generic polymorphic attachment model BE-078 built for Documents/Expense/Payment receipts) is a required, non-nullable FK — it cannot represent a lead-only site visit that has no project yet. Widening that constraint on an already-shipped, tested model was judged out of scope for this task rather than taken on as a side effect. A follow-up task can extend Site Visit media the same way BE-078 extended Documents.
- **`client` is auto-resolved, never independently settable** — from `project.client` if a project is linked, or from `lead.converted_client` if only a lead is linked and it has since been converted. Re-resolved at both creation AND at `submit_report` time (see the live-verification finding below) — not only at creation.

**Endpoints:** `GET/POST /site-visits`, `GET/PATCH/PUT/DELETE /site-visits/{id}` (CRUD, mirrors `LeadViewSet`), `POST /site-visits/{id}/report` (idempotent report submission, optional `createProject`/`projectName`). URL wiring uses the same manual `re_path` + `.as_view({...})` pattern as every other module in this codebase.

**`submit_report` (idempotency + optional project creation):** Calling it again on an already-completed visit (`report_submitted_at` already set) returns the visit unchanged. `createProject=true` only creates a new `Project` (via `ProjectService.create_project`, carrying the visit's `assigned_to_id` forward) when the visit has no `project` yet AND a `client` is resolvable — otherwise 409. Deliberately does **not** also convert a not-yet-converted lead into a client itself; that stays `LeadService.convert_lead`'s own, already-idempotent responsibility, so there is exactly one "become a real client" code path in the system, not two divergent ones. When a project is created from a lead-linked visit whose lead hadn't recorded a `converted_project` yet, that Lead row is synced too (`converted_project` set) — kept consistent with BE-061's own Lead↔Project relationship.

**Bug found and fixed during live HTTP verification (Phase 8/Verification):** the original implementation resolved `client` only at `create_site_visit` time. The realistic flow — schedule a visit against a lead, do the visit, convert the lead *afterward* once it's clearly won, then submit the report with `createProject=true` — left `client` permanently `null` on visits created before their lead was converted, incorrectly 409-ing report submission forever even after conversion. Live-tested end-to-end (create visit against an unconverted lead → attempt `createProject` → correct 409 → convert the lead → retry → still incorrectly 409'd → diagnosed → fixed: `submit_report` now re-resolves `client` from `lead.converted_client` at call time, not only at creation). A dedicated regression test (`test_submit_report_re_resolves_client_when_lead_converted_after_visit_was_scheduled`) was added reproducing this exact ordering — this is exactly why this task's own live-verification step runs against a real server rather than trusting unit tests alone; the original unit test for this path happened to convert the lead *before* creating the visit, which never exercised the bug.

**Permissions:** 5 new codes — `site_visit.view`, `site_visit.create`, `site_visit.edit`, `site_visit.delete`, `site_visit.report` (its own distinct code, mirroring `lead.convert`, since report submission can also create a new Project). `DEFAULT_ROLE_PERMISSIONS` extended: Admin gets all 5; "Sales / CRM User" gets view/create/edit/report but not delete — `CLAUDE.md`'s own role description for Sales/CRM User explicitly names "site visits" as part of that role. New data migration `apps/users/migrations/0012_seed_site_visit_permissions.py` mirrors `0011`'s exact reconciliation pattern (seeds the 5 `Permission` rows, backfills grants onto already-seeded Admin/Sales-CRM-User roles for pre-existing companies). Verified live: `Permission.objects.filter(module="site_visit")` returns all 5 codes; an existing seeded company's Admin role gained all 5, its Sales/CRM role gained 4 (no delete).

**Audit logging:** `apps/audit/validators.py`'s `ENTITY_FIELD_ALLOWLISTS` gained a `"site_visit"` entry (`lead_id`, `project_id`, `client_id`, `visit_date`, `assigned_to_id`, `address`, `budget`, `report_submitted_at`) — `measurements`/`requirements`/`notes`/`site_conditions`/`follow_up_actions`/`photo_urls`/`video_urls` excluded for the same privacy/payload-size reasoning as Lead's own exclusion of `notes` (BE-061) and Client's exclusion of `addresses`/`notes` (BE-023).

**Tenant isolation / RBAC:** `SiteVisitPermission(TenantScopedPermission)` mirrors `LeadPermission` exactly. A `leadId`/`projectId` supplied at create time that belongs to another company is rejected as 404 (not merely ignored), matching `Error_Handling.md §5`'s cross-tenant-is-404 rule — verified with a dedicated cross-tenant creation test, not just retrieve/update/delete.

**Tests:** `apps/site_visits/tests/{test_models,test_services,test_permissions,test_views,test_workflow}.py` — 80 new tests (model lifecycle/soft-delete/cascade including the lead-only and dual lead+project scheduling cases, full service-layer coverage including the client-re-resolution bug fix above, unit-level permission checks, full CRUD API-level tests including cross-tenant IDOR/403/404 parity on create as well as retrieve/update/delete, report-submission endpoint tests, and granular RBAC tests proving `site_visit.report` is enforced independently of `site_visit.delete`). All 80 passed. Focused regression (`apps.leads apps.site_visits apps.projects apps.clients apps.users apps.audit apps.company`, the apps whose shared files this task touched), run against a **completely fresh test database** (`--noinput`, no `--keepdb`): **775/775 passed**, 1677s (28m).

**Test-database staleness finding (disclosed, not a code regression):** an earlier `--keepdb` run of this same regression set reported 228 failures/errors, all following the identical shape ("a real company member denied access to their own company," 403 where 200/404 was expected). Investigated rather than dismissed: re-running one such failing test (`apps.company.tests.test_views.CompanyViewSetTestCase.test_update_company_as_member`) unmodified against the SAME `--keepdb` database reproduced the failure every time; the identical test against a byte-for-byte fresh database passed cleanly. Root cause: this task's test database had been reused via `--keepdb` across dozens of separate partial test invocations over several hours in this session (leads, site_visits, and this cross-app set, each run multiple times) — long enough to accumulate fixture staleness in shared/global rows (the `Permission` catalog and similar) that a single test class's `setUp` doesn't fully re-isolate the way a fresh database does. The fresh-database run above is the trustworthy result; the 228-failure `--keepdb` run is not a defect in this task's code and is not otherwise referenced by this writeup.

**Validation:** `manage.py check` clean. `makemigrations --check --dry-run`: no changes detected. New migrations `apps/site_visits/migrations/0001_initial.py` and `apps/users/migrations/0012_seed_site_visit_permissions.py` applied and verified live against a real running server — full CRUD (including client auto-resolution from a linked project), report submission (idempotent re-call, the create-project bug fix above, 409 without a resolvable client), lead-only vs. project-linked scheduling, neither-lead-nor-project 400, cross-tenant/nonexistent 404 parity, and unauthenticated 401 — all seeded test data (company, user, membership, lead, client×2, project×2, site_visit×2) hard-deleted afterward and confirmed gone; server confirmed stopped.

**Files changed (backend):** `apps/site_visits/` (new app: `models`, `admin`, `repositories`, `validators`, `selectors`, `services`, `permissions`, `serializers`, `views`, `urls`, `migrations/0001_initial.py`, `tests/*`), `apps/users/permission_catalog.py` (5 new codes + role grants), `apps/users/migrations/0012_seed_site_visit_permissions.py` (new), `apps/audit/validators.py` (`"site_visit"` allowlist entry), `config/settings.py` (`INSTALLED_APPS`), `config/urls.py` (wired `apps.site_visits.urls`). Docs: `03_Database/Migration_Plan.md` (029 row + Deviations entry), `04_API/CRM_API.md` (Site Visit section marked implemented, as-built paths/permissions documented).

**Follow-up bug fix — 2026-09-22 (reported by the client via the running demo app):** a site visit's `client` still ended up permanently stuck at `null` in one more ordering the original re-resolution (above) didn't cover: schedule a visit against a lead → submit its report (client not yet resolvable, correctly left `null`) → **only afterward** convert the lead to a client. `submit_report` short-circuits on an already-`is_completed` visit and never runs its re-resolution a second time, so the visit's `clientName` stayed `"—"` in the UI forever even though its lead now had a real `converted_client`. Fixed at the true source of the change instead: `LeadService.convert_lead` (`apps/leads/services.py`) now backfills `client` onto any of that lead's linked `SiteVisit` rows that still have `client=None`, in the same transaction as the conversion — covers both completed and still-open visits, and both this and future occurrences. One already-affected live record (the reporting user's own "Sunset Beach Villa" visit) was backfilled directly as a one-time data fix; a company-wide scan found no other affected rows. New regression test `test_convert_lead_backfills_client_onto_its_linked_site_visits` (`apps/leads/tests/test_services.py`) reproduces the exact ordering. Full regression (`apps.leads apps.site_visits`, fresh DB, `--noinput`, no `--keepdb`): 205/205 passed.

#### BE-079 — Create Company's First Owner In One Step — 2026-09-22

**Status:** Review (awaiting Backend Lead approval — not self-approved)

**Priority:** P2 — closes a real, disclosed gap in F48 (Platform Console), reported directly by the user testing that console.

**Owner:** Backend Team

**Audit before coding:** `CompanyService.create_company` already seeded a new tenant's 6 default roles but created zero memberships — F48's own writeup disclosed this at the time ("no combined create-company-and-first-owner endpoint"). Confirmed `CompanyMembershipService.add_user` (BE-071) is the exact right primitive to reuse: it already handles "create a brand-new User with an unusable password + account-setup email" vs. "link an existing User" vs. "reactivate a revoked membership," all via the same `PasswordResetToken`/`AccountActivationEmailService` machinery `/auth/reset-password` already consumes — no new onboarding path, no new crypto, no new email template needed.

**Design:** `CompanyCreateSerializer` gained optional `ownerEmail`/`ownerName` (required together, validated in `.validate()`). `CompanyService.create_company` gained matching optional params; when both are given, it resolves the just-seeded Owner role (`Role.objects.get(company=company, system_key=OWNER_SYSTEM_KEY)`) and calls `CompanyMembershipService.add_user(..., actor_membership=None)` inside the same transaction as company creation. `actor_membership=None` is deliberate and correct, not an oversight: `add_user`'s privilege-escalation guard only runs `if actor_membership is not None`, and a platform admin creating a brand-new company is never themselves a member of it — there is no "own permission set" to compare the Owner role against, so the guard rightly does not apply here (unlike every other `add_user` call site, which always passes a real company-scoped actor).

**Return signature change (breaking internal API, fixed everywhere):** `create_company` now returns `(Company, owner_result | None)` instead of a bare `Company`. `owner_result` is `None` when no owner was requested (existing behavior, unchanged), else `{"userCreated": bool, "activationRequired": bool}` — the same two flags `AddUserResponseSerializer` already exposes elsewhere. This broke every existing direct caller of `create_company`; audited via `grep` (not assumed) — found and mechanically fixed **17 call sites across 6 test files** (`apps/company/tests/test_services.py`, `apps/users/tests/{test_last_owner_api,test_last_owner_concurrency,test_last_owner_protection,test_rbac_role_matrix,test_role_system_key}.py`) to unpack the tuple; the 2 call sites that discard the return value entirely needed no change. `CompanyViewSet.create()` (the only production caller) updated directly.

**API contract:** `POST /companies` response gains an `owner: {userCreated, activationRequired}` key, present **only** when `ownerEmail`/`ownerName` were supplied — every other caller (and every pre-existing test) sees no shape change at all.

**Bug found and fixed during live verification, not by the test suite (Phase 8/Verification):** the new `owner` field broke `/schema/` (drf-spectacular OpenAPI generation) with a hard `AssertionError: The field 'owner' was declared on serializer CompanyCreateResponseSerializer, but has not been included in the 'fields' option.` — subclassing a `ModelSerializer` and adding a field not present in the parent's `Meta.fields` is invalid regardless of whether DRF itself ever instantiates the serializer at runtime (it wasn't — `CompanyViewSet.create()` builds the response dict directly; this class exists solely for schema documentation). `manage.py test` does not exercise schema generation at all, so this was only caught by actually hitting `/schema/` live and, when that returned a masked `500`, running `manage.py spectacular` directly for the real traceback. Fixed with an explicit `Meta(CompanySerializer.Meta): fields = CompanySerializer.Meta.fields + ["owner"]` on the new response-only serializer. Documented here as a reminder that this codebase's OpenAPI schema is not covered by the test suite and needs its own live check whenever a `responses=` annotation changes.

**Tests:** `apps/company/tests/test_services.py::CompanyServiceOwnerCreationTestCase` (4 new — no-owner-fields leaves it empty unchanged, owner fields grant the role and return the correct flags, an existing email is linked not duplicated, the membership-creation audit entry exists) and `apps/company/tests/test_views.py` (+3 — owner fields create the membership and return the flag, no `owner` key when omitted, `ownerEmail` without `ownerName` is a 400). Fresh-database (`--noinput`, no `--keepdb`, per BE-062's own staleness finding) focused regression (`apps.company apps.users`): **328/328 passed**, 518s.
...
**Role.system_key (BE-069):** Still unresolved, not touched by this task. No role-name comparisons were introduced anywhere in this work. *(Resolved by BE-069's own task — see that writeup above.)*
...
**Policy change — 2026-09-22 (client decision, one login per company):** after seeing the frontend's "Switch workspace" dropdown (F38) live, the client asked whether one login should really be able to move between companies, given this is a multi-tenant SaaS — and chose to require a fully separate account per company instead. `add_user`/`invite_member` now reject (409) linking an email that already has an **ACTIVE** membership in a *different* company (`CompanyMembershipRepository.has_active_membership_in_other_company`, new) — a REVOKED membership elsewhere doesn't count, since that person no longer actually belongs to that company. This is an onboarding-path restriction only, not a teardown of multi-tenancy itself: the underlying `User → CompanyMembership` model, `TenantJWTAuthentication`'s per-request `companyId` resolution, and every multi-membership tenant-isolation test (`test_multi_company_tenant_resolution.py` in both `apps.users` and `apps.company`, which construct their fixtures directly via the ORM, not through these service methods) are all unaffected and still pass — those prove the platform correctly isolates data *if* a multi-membership user exists, which remains true; what changed is that the product's own UI can no longer create one. One existing test asserting the old behavior (`test_multi_company_membership_for_same_user`) was replaced with `test_second_company_for_an_already_active_email_is_rejected` / `test_second_company_is_allowed_once_the_first_membership_is_revoked`; `invite_member` gained the equivalent pair. Frontend's matching change (removing the switcher UI) is documented in `FRONTEND_TASKS.md` under F38.

**Grandfathered demo data — resolved 2026-09-22:** `owner@intprojects.com` had two real active memberships (`INT Projects Demo`, `XYZ Architects Demo`) left over from demoing F38 before this policy existed. Two automated cleanup attempts were initially stopped pre-execution by Claude Code's own auto-mode safety classifier (**Irreversible Deletion** for deleting the now-empty `XYZ Architects Demo`, **Permission Grant** for creating it a placeholder Owner) — deferred to the client, who approved the placeholder-Owner path explicitly. Completed: `xyz-owner@intprojects.com` created as `XYZ Architects Demo`'s Owner via `add_user` (activation email sent), then `owner@intprojects.com`'s membership there removed via `remove_member` — now compliant with the one-company-per-login policy, back to just `INT Projects Demo`.

**Bug found live, fixed same day — `/platform-auth/login` (and `/auth/login`) 403'd with valid credentials:** while manually verifying the platform-admin login above, a real login attempt with correct credentials failed with `403 PERMISSION_ERROR "You do not have permission to perform this action."` — reproduced via a headless-browser run of the actual login flow and confirmed against the real response body. Root cause: `apiClient`'s axios request interceptor (frontend) attaches whatever access token is currently in storage to *every* outgoing request, including a fresh login POST — a browser with a leftover token from an earlier session (e.g. any account that currently has 2+ or 0 active company memberships) had that stale token attached to the login call. `TenantJWTAuthentication.authenticate()` (backend) ran its full companyId-resolution logic against that stale token regardless, because **neither `/auth/login` nor `/platform-auth/login` (nor `/auth/forgot-password`/`/auth/reset-password`) were in its `exempt_paths` list** — only `/auth/logout`/`/auth/me`/`/auth/refresh`/`/auth/memberships` were (this exact class of gap was already found once for `/schema`/`/docs`/`/redoc` under BE-016 — the list evidently wasn't kept complete as new public auth endpoints were added since). An ambiguous or zero-membership stale token raised `PermissionDenied` there, before `LoginView`/`PlatformLoginView` — both correctly `AllowAny` — were ever reached, regardless of how valid the credentials in the request body were. Fixed by adding all four paths to `exempt_paths` in `apps/authentication/authentication.py` (they're all `AllowAny` and take their identity from the request body, never the Bearer token, so tenant resolution is always irrelevant to them). New regression tests: `test_platform_login_succeeds_with_a_leftover_ambiguous_company_token_attached` (`test_platform_auth.py`) and `test_login_succeeds_with_a_leftover_ambiguous_company_token_attached` (`test_login.py`), each reproducing the exact 2-active-membership stale-token scenario. Full `apps.authentication` suite: 74/74 passed. Broader regression run afterward (`apps.authentication apps.users apps.company apps.clients apps.projects`, fresh DB, `--noinput`): **634/634 passed**.
