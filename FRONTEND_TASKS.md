...
**Bug fix — 2026-09-22 (reported by the client: "Create Invoice"/"Add member" buttons unreadable, "fix everywhere"):** every `variant="primary"` `<Button>` app-wide (Login, Create Invoice, Add member, and every other primary action) rendered with unreadable text — dark-on-dark in light theme, light-on-light in dark theme — because its text color was silently missing from the DOM entirely. Root cause: `tailwind-merge` (via the shared `cn()` helper in `src/lib/utils.ts`) only knows Tailwind's own built-in color/font-size scale. It has no way to know this project's custom `theme.extend` adds two unrelated `text-*` scales — text color (`text-ink-text`, `text-text-primary`, ...) and text size (`text-body`, `text-h1`, ...) — so it treated every `text-*` class passed to a component as one ambiguous group and silently dropped all but the last one. `Button`'s `variant` class (`text-ink-text`) is concatenated before its `size` class (`text-body`), so `text-ink-text` always lost and the button fell back to inheriting `--text-primary` — a color deliberately close to `--ink-bg` in both themes, since it's meant for body text on the *page* background, not a fill button's own background. This was **not a stale-tab/cache issue** (ruled out first, since it survived a hard refresh) and not something a screenshot alone could diagnose with confidence — confirmed via a headless Chrome check of the actual computed DOM (`getComputedStyle`), which showed `text-ink-text` genuinely absent from the rendered `className`. Fixed by registering the project's custom color and font-size scales with `extendTailwindMerge` so it can tell the two `text-*` groups apart. Since the fix lives in the single shared `cn()` helper, it corrects every affected button across the whole app at once, not just the two the client pointed at. Full test suite re-run afterward: 373/373 passed (2 suites / 8 tests still skipped for the pre-existing, unrelated jsdom+Radix reason noted below).
...
**F38 superseded — 2026-09-22 (client decision):** after seeing the switcher live and asking whether one login should really be able to move between companies at all, the client chose to disable cross-company switching entirely rather than keep it — see `BACKEND_TASKS.md`'s matching entry for the full policy and reasoning. The "Switch workspace" section was removed from `Header.tsx`'s user menu (dead code — `handleSwitchCompany`, its `Building2`/`Check` icon imports — removed with it). `activeCompanyStore`/`useCurrentCompanyId` themselves are unchanged and still load-bearing: they're what resolves `companyId` for every request even for a single-membership user, and `useCurrentCompanyId.test.tsx`'s multi-membership coverage stays valid as a test of that resolution logic working correctly if a multi-membership user exists (e.g. already-seeded data, or a future admin override) — the change is that the product's own onboarding paths can no longer create one. Frontend tests: 373/373 passed. `tsc --noEmit`: clean.
...
| F49 | UX pass requested by the manager ("make it user-friendly") — forgot-password flow, show/hide password + autofocus, per-page tab titles, error boundary + real 404 page, dashboard "Get started" checklist | Review |
...
- Visual QA: **PENDING** at original ship — no browser tooling available in this environment; live HTTP verification above covers the functional contract, and a real platform-admin demo account was created for the user to try the console themselves.
- **Bug found live, fixed 2026-09-22 — Companies list rendered completely blank after a real login:** the user logged in successfully at `/platform/login` and landed on `/platform/companies`, but the page below the header was entirely empty — no table, no "New Company" button, nothing, and no error either. This is exactly the class of bug the "Visual QA: PENDING" note above predicted: purely client-side, invisible to every backend/HTTP-level check this task had actually run. Root-caused with a headless Chrome session driving the real login flow (`playwright-core` against the system Chrome install, now available in this environment) and watching the network log: after login, **zero** request for `PlatformCompaniesListPage`'s lazy chunk or `GET /companies` ever fired — the nested `<Routes>` inside the `/platform/*` route's element matched nothing at all. Cause: that inner `<Routes>` used **absolute** child paths (`/platform`, `/platform/companies`, ...), copying the pattern the tenant `/*` block uses successfully — but that pattern only "works" there by coincidence, since `/*`'s own matched prefix is empty, so absolute and parent-relative paths happen to be identical. Under `/platform/*`, the matched prefix (`/platform`) is genuinely stripped before the nested `<Routes>` sees the remaining path, so its absolute-path children never matched anything, silently falling through to the catch-all's `<Navigate to="/platform/companies" replace>` — which was already the current URL, so nothing visibly changed and nothing ever rendered. Fixed by making the platform block's inner routes relative to its own mount point (`index`, `companies`, `companies/:companyId`, `*`), the standard React Router v6 nested-routing convention — confirmed via the same headless flow: `GET /companies` now fires and the table renders all 7 tenant companies correctly. The tenant `/*` block was deliberately left untouched (it works, `/*`'s prefix makes the bug moot there, and it's a much larger, already-proven route tree not worth the risk of touching for this fix). Frontend suite: 373/373 passed.
- **Feature added 2026-09-22 (BE-080) — "Set Owner Password" on the Company Detail page:** the client pointed out there was no way at all to get a fresh Owner logged in, since the account-setup email only prints to the dev server's own console. New `components/platform/SetOwnerPasswordDialog.tsx` (mirrors `MarkLostDialog`'s structure) opens from a new button on `PlatformCompanyDetailPage.tsx`'s header (alongside Edit/Delete) — a single password field, submits to `POST /companies/{id}/owner/set-password`, shows every backend validation issue inline (Django's real password-strength messages, not reinvented client-side), and confirms success with a toast naming who the password was set for. `lib/api/company.ts` gained `setCompanyOwnerPassword`. Verified end-to-end through the real browser (login → open a company → click the button → fill the field → submit → confirmed the new password actually logs in via a follow-up `/auth/login` call). New tests: `SetOwnerPasswordDialog.test.tsx` (4). `tsc --noEmit`: clean. Frontend suite: 377/377 passed.
- **Gap found live and closed same day — no visibility into who the Owner even was:** the client pointed out that neither the Company Detail page nor the Set Owner Password dialog ever showed the current Owner's email/name — nothing to confirm before acting. Added a new "Owner" card to `PlatformCompanyDetailPage.tsx` (loading skeleton, an honest `EmptyState` for a company with no active Owner, and — on success — name/email plus an Active/Needs setup `Badge` driven by the backend's real `hasUsablePassword` flag) sourced from the new `GET /companies/{id}/owner`. The "Set Owner Password" trigger moved from the page header into this card, and `SetOwnerPasswordDialog` now names the actual Owner in its description instead of only the company. `lib/api/company.ts` gained `getCompanyOwner`; `queryKeys.ts` gained `platformCompanyKeys.owner`, invalidated on a successful password set so the badge flips to Active immediately. Verified live for both the has-an-owner and no-owner cases through the real browser. `tsc --noEmit`: clean. Frontend suite: 377/377 passed.

**Implementation notes (F49, 2026-09-24):** driven by a real audit of the running app (headless Chrome, desktop + phone width) rather than guesswork — visual design and mobile layout were already sound; the friction was all in first-run and recovery paths.
- **Forgot password** was missing from the frontend entirely (the backend's `POST /auth/forgot-password` had no page or link, so a locked-out user had no way back in). New `ForgotPasswordPage` at `/forgot-password`, linked from the login page; its confirmation copy is deliberately noncommittal ("if an account exists…") to match the backend's anti-enumeration response.
- New shared `PasswordInput` (show/hide toggle, `aria-pressed`, ref-forwarding so `register()` works) used on both login pages, Set Password, and the Owner-password dialog. Email field auto-focuses on the login pages.
- **Tab titles**: every tab read "Architecture SAS". New `PageTitle` (route→title table, first match wins) sets `Leads · INT Projects` etc.; `index.html` renamed to match the product brand.
- **Error handling**: added a route-keyed `ErrorBoundary` (a render crash previously left a blank white page with no way out — the exact symptom hit twice during deployment debugging) and a real `NotFoundState` for unknown URLs, which used to silently bounce to the dashboard.
- **Dashboard "Get started" checklist** for new companies (add a client / create a project / prepare a quotation / invite your team): ticks itself from real data, hides when complete or dismissed (per-company, `localStorage`), and omits any step the caller lacks permission to read rather than showing it forever undone. Net Profit card no longer claims "Profitable this period" at exactly ₹0.
- Tests: 25 new (PasswordInput, ForgotPasswordPage, PageTitle, ErrorBoundary, GettingStartedCard, +1 LoginPage). Frontend suite: 402 passed, 8 skipped (unchanged baseline). `tsc --noEmit`: clean. Verified visually in a real browser incl. a genuinely empty company.
