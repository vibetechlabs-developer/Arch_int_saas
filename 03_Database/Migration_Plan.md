...
| 029 | `create_site_visits` | `site_visit` | `company`, `leads` (nullable), `projects` (nullable), `clients` (nullable), `user` (assigned_to, nullable) | **Phase 3 table, built ahead of schedule 2026-09-15 — see Deviations below.** Depends on `lead` (028), so ordered directly after it. |
...
- **`site_visit` (029), added and implemented 2026-09-15 (BE-062), same day and same governance basis as `lead` (028) above.** `01_Business/FRS.md §9` documents a real field list for this table ("client, project/lead, visit date, assigned person, address, measurements, requirements, photos, videos, notes, budget, site conditions, follow-up actions") — unlike Lead, so nothing here is invented except: (a) no `status` field — FRS/CRM_API.md document no status vocabulary for Site Visit, so `report_submitted_at` (nullable) alone carries the scheduled-vs-completed distinction rather than a parallel invented enum; (b) `photo_urls`/`video_urls` are plain JSONField URL lists, not the `apps.common.storage`-backed upload plumbing BE-078 built for Documents/Expense/Payment receipts — `Document.project` is a required (non-nullable) FK and so cannot represent a lead-only site visit that has no project yet, and widening that constraint on an already-shipped, tested model was judged out of scope for this task; a follow-up can extend it the same way BE-078 did for the other modules. See `apps/site_visits/models.py`'s own docstring for the full reasoning on both points. Implemented as `apps/site_visits/migrations/0001_initial.py` (Django's own per-app auto-numbering; "029" above is this plan's cross-app-ordering placeholder only).
